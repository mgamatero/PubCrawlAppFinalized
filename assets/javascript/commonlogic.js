...
//suggestions
    ...
    //    console.log(deleted.includes(snap.val().suggestion))
        if(snap.val().group == crawlSync) {
            if(snap.val().group == crawlSync) {
                $('ol').append("<li class='barsPicked' id='"+snap.val().randomID+"' dataIndex ='"+snap.val().randomID+"'>" + snap.val().suggestion + "<button class='x-buttons' id='"+snap.val().randomID+"'>X</button></li>")
            }
        }
    }, function (errorObject){
        console.log("The read failed:" +errorObject)
        ...
        */
        ...
        $.ajax({
            ...
            $(".addMeToCrawl").on("click", function () {
              //  suggestionsBox = database.ref('/suggestionsBox')
